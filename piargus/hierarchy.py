...
class Hierarchy:
    ...
    def __init__(self, tree=None, indent='@', total_code="Total"):
        if not isinstance(tree, HierarchyNode):
            tree = HierarchyNode(total_code, data=tree)
    ...
    def __repr__(self):
        return (f"{self.__class__.__name__}"
                f"({self.root.to_dict()}, {self.indent}, total_code={self.total_code})")
    ...
    @classmethod
    def from_hrc(cls, file, indent='@', total_code="Total"):
        ...
        root = from_indented(file, indent=indent, node_factory=HierarchyNode, root_name=total_code)
    ...
    @classmethod
    def from_rows(cls, rows: Iterable[Tuple[str, str]], indent='@', total_code="Total"):
        """Construct from list of (code, parent) tuples."""
        return cls(from_rows(rows, HierarchyNode, root_name=total_code), indent=indent)
